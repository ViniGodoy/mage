...
public class Shader {
    ...
    public Shader setAttribute(String name, ArrayBuffer buffer) {
        ...
        if (attribute == -1) {
            Logger.getLogger(getClass().getCanonicalName())
                  .info("Attribute does not exists: " + name);
        } else if (buffer == null) {
    ...
    private int findUniform(String name) {
        ...
        if (uniform == -1) {
            Logger.getLogger(getClass().getCanonicalName())
                  .info("Uniform não encontrado: " + name);
        }
        return uniform;
    }

    /**
     * Define o valor de um uniforme deduzindo seu tipo
     * @param name Nome do uniforme
     * @param value Valor do uniforme
     * @return O próprio shader
     * @throws UnsupportedOperationException Se a classe não for suportada
     */
    public Shader setUniformObj(String name, Object value) {
        if (value.getClass() == Matrix3f.class) {
            setUniform(name, (Matrix3f) value);
        } else if (value.getClass() == Matrix4f.class) {
            setUniform(name, (Matrix4f) value);
        } else if (value.getClass() == Vector3f.class) {
            setUniform(name, (Vector3f) value);
        } else if (value.getClass() == Vector2f.class) {
            setUniform(name, (Vector2f) value);
        } else if (value.getClass() == Vector4f.class) {
            ...
            throw new UnsupportedOperationException(value.getClass().getCanonicalName() + " not supported!");
        }
